...
- **Video format detection** — the Vita decodes only H.264/AAC in an MP4, and anything else used to fail inside `sceAvPlayer` with no message, so the scene silently did nothing. The engine now reads the file's own bytes, names the container (MPEG-1/2, AVI, WMV, Matroska, Ogg, FLV, RealMedia) and says what to convert it to, then continues the script instead of stalling. Dropping a converted `<name>.mp4` beside the original makes it play, with no change to the script
...
game icons from the archive, automatic video conversion, per-game touch presets,
