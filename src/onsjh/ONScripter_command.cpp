...
int ONScripter::savescreenshotCommand()
{
    ...
    const char *buf = script_h.readStr();
    SDL_RWops *rwops = (SDL_RWops*)SDL_RWFromFile(buf, "wb");
...
int ONScripter::mp3stopCommand()
{
    ...
    if (Mix_PlayingMusic() == 1 && mp3fadeout_duration > 0){
        ...
        timer_bgmfade_id = (SDL_TimerID)SDL_AddTimer(20, bgmfadeCallback, 0);
...
int ONScripter::mp3Command()
{
    ...
    if (buf[0] != '\0'){
        ...
        if (mp3fadein_duration > 0) {
            ...
            mp3fade_start = SDL_GetTicks();
            timer_bgmfade_id = (SDL_TimerID)SDL_AddTimer(
                20, bgmfadeCallback, (void*)&timer_bgmfade_id);
...
#if defined(IOS) || defined(ANDROID) || defined(WINRT) || defined(WINRT) || defined(PSV) 
    if (num_fingers > 1)
...
int ONScripter::getscreenshotCommand()
{
    ...
    if (screen_dirty_flag) {
        ...
        SDL_RenderReadPixels(renderer, &render_view_rect, 0, screenshot_surface->pixels, screenshot_surface->pitch); // fix here
...
int ONScripter::btndefCommand()
{
    if (script_h.isName( "bclear" )){
        ...
        if ( buf[0] != '\0' ){
            ...
            SDL_SetSurfaceBlendMode((SDL_Surface*)(btndef_info.image_surface), SDL_BLENDMODE_NONE);
        }
    }
...
static SDL_Texture* createMaximumTexture(SDL_Renderer *renderer, SDL_Rect &blt_rect, const SDL_Rect &src_rect, SDL_Surface *blt_surface,
    ...
    SDL_Texture *blt_texture = SDL_CreateTextureFromSurface(renderer, (SDL_Surface*)surface);
    SDL_FreeSurface(surface);
    return blt_texture;
}
...
int ONScripter::bltCommand()
{
    ...
    if (sx >= 0 && sy >= 0 && sw > 0 && sh > 0) {
		...
		else 
        {
			...
			dst_rect.h = blt_texture_src_rect.h;
		}
#endif
