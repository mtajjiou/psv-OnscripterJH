...
unsigned char _binary_res_head_bin_start;
unsigned char _binary_res_head_bin_size;
...
int VitaPackage::Install()
{
    int ret = makeHeadBin();
    if (ret < 0) {
		printf("Can't make head.bin for : 0x%08X\n", path_.c_str(), ret);
		return 0;
    }
    ...
    if (ret < 0) {
		printf("Can't Promote %s: scePromoterUtilityPromotePkgWithRif() = 0x%08X\n", path_.c_str(), ret);
		return 0;
    }
    ...
    do {
        ...
        if (ret < 0) {
			printf("Can't Promote %s: scePromoterUtilityGetState() = 0x%08X\n", path_.c_str(), ret);
			return 0;
        }
    ...
    if (ret < 0) {
		printf("Can't Promote %s: scePromoterUtilityGetResult() = 0x%08X\n", path_.c_str(), ret);
		return 0;
    }
